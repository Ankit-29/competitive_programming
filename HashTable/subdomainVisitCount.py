...
"leetcode.com", and at the lowest level, "discuss.leetcode.com". When we visit a domain like "discuss.leetcode.com", we will also visit 
the parent domains "leetcode.com" and "com" implicitly.

Now, call a "count-paired domain" to be a count (representing the number of visits this domain received), followed by a space, followed by 
...
def subdomainVisits(cpdomains: List[str]) -> List[str]:
        ...
        for cpdomain in cpdomains:
            ...
            for fragment in fragments[::-1]:
                key = (fragment+"."+last) if(last!="") else fragment
                last = key
                if(key in Hash):
                    Hash[key] += int(val)
                else:
        ...
        for key in Hash.keys():
            output.append(str(Hash[key]) + " " + key)
